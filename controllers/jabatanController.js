...
exports.addJabatan = (req, res) => {
  ...
  const sql =
    "INSERT INTO jabatan (nama_jabatan, deskripsi_jabatan) VALUES (?, ?)";
  db.query(sql, [nama_jabatan, deskripsi_jabatan], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
    res
      .status(201)
      .json({ message: "Jabatan berhasil ditambahkan", id: result.insertId });
  });
};
...
exports.updateJabatan = (req, res) => {
  ...
  const { nama_jabatan, deskripsi_jabatan } = req.body;
  const sql =
    "UPDATE jabatan SET nama_jabatan=?, deskripsi_jabatan=? where id_jabatan=?";
  db.query(sql, [nama_jabatan, deskripsi_jabatan, id], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
