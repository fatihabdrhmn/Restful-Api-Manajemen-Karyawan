...
exports.loginAdmin = (req, res) => {
  ...
  db.query(sql, [username, password], (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    if (results.length === 0) return res.status(401).json({ message: "Login gagal, username atau password salah" });

    const admin = results[0];
    res.json({ message: "Login berhasil", admin: { id_admin: admin.id_admin, nama: admin.nama, username: admin.username } });
  });
};
