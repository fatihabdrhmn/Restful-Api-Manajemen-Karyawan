...
exports.addDepartemen = (req, res) => {
  ...
  const sql = "INSERT INTO departemen (nama_departemen, deskripsi_departemen) VALUES (?, ?)";
  db.query(
    sql, 
    [
    nama_departemen, 
    ...
    (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
    res.status(201).json({ message: "Departemen berhasil ditambahkan", id: result.insertId });
  });
};
